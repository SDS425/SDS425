...
```{r}
county_merged_data = with(cleaned_data_list, merge(gasprice[laws_incentives, on = .NATURAL],
                                                   climate_attitudes[election, on = .NATURAL]))

GEOID_merged_data = with(cleaned_data_list, Reduce(merge, list(POI_counts, road_mileage, rest_stops)))

merged_data = with(cleaned_data_list, EVstations[census, on = .NATURAL] |>
                     replace_NA(value = 0, cols = names(EVstations)))
merged_data = GEOID_merged_data[county_merged_data[merged_data,
...
merged_data[, `:=`(STATEFP = str_pad(STATEFP, 2, pad = 0),
  ...
  setcolorder(c("GEOID", "state", "STATEFP", "COUNTYFP"))
